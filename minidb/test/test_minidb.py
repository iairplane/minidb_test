...
def test_store_exit_with_exception_and_vacuum_false(tmp_path):
    class Foo(minidb.Model):
        value = int

    dbfile = tmp_path / 'minidb_exit.db'
    store = minidb.Store(str(dbfile), vacuum_on_close=False)
    store.register(Foo)
    store.__exit__(RuntimeError, RuntimeError('x'), None)


def test_store_utility_methods_and_deserialize_identity():
    class Foo(minidb.Model):
        value = int

    with minidb.Store(debug=True, smartupdate=True) as db:
        db.register(Foo)
        item = Foo(value=10).save(db)

        # save_or_update -> update path
        item.value = 11
        db.save_or_update(item)

        # no-change smart update branch
        item.value = 11
        db.save_or_update(item)

        assert db.deserialize(1, int) == 1
        db.commit()
        db.vacuum()


def test_query_order_group_limit_and_select_lambda():
    class Foo(minidb.Model):
        name = str
        score = int
    ...
    with minidb.Store(debug=True) as db:
        db.register(Foo)
        Foo(name='a', score=1).save(db)
        Foo(name='a', score=3).save(db)
        Foo(name='b', score=2).save(db)

        rows = list(Foo.query(
            db,
            select=lambda c: c.name // c.score.sum('total'),
            where=lambda c: c.score >= 1,
            group_by=lambda c: c.name,
            limit=2,
        ))
        assert len(rows) == 2
...
def test_rowproxy_repr_keys_and_index_access():
    class Foo(minidb.Model):
        value = int
    ...
    with minidb.Store(debug=True) as db:
        db.register(Foo)
        Foo(value=9).save(db)
        row = next(Foo.query(db, Foo.c.id // Foo.c.value))
        assert isinstance(repr(row), str)
        assert row.keys() == ['id', 'value']
        assert row[0] == row.id


def test_operation_edge_branches_and_error_paths():
    class Foo(minidb.Model):
        value = int
    ...
    with minidb.Store(debug=True) as db:
        ...
        op = minidb.Operation(Foo.c.value, '+', 2, brackets=True)
        sql, args = op.tosql()
        assert sql == 'value + ?'
        assert args == [2]

        outer = minidb.Operation(op, 'AND', Foo.c.value == 1)
        outer_sql, _ = outer.tosql(brackets=True)
        assert '(' in outer_sql and ')' in outer_sql

        # __floordiv__ on binary operation should fail
        with pytest.raises(ValueError):
            _ = op // Foo.c.id

        # argtosql converter branch
        class Point(object):
            def __init__(self, x):
                self.x = x

        @minidb.converter_for(Point)
        def convert_point_for_branch(v, serialize):
            return str(v.x) if serialize else Point(int(v))

        conv_sql, conv_args = minidb.Operation(Foo.c.value).argtosql(Point(7))
        assert conv_sql == '?'
        assert conv_args == ['7']

        # _get_class failure branch
        with pytest.raises(ValueError):
            minidb.Operation(123).query(db)


def test_sequence_and_columns_helpers():
    class Foo(minidb.Model):
        a = int
        b = int
    ...
    with minidb.Store(debug=True) as db:
        ...
        seq = minidb.columns(Foo.c.a, Foo.c.b)
        assert 'Foo.a' in repr(seq)

        seq2 = Foo.c.a // Foo.c.b
        seq2 // Foo.c.id
        assert len(seq2.args) == 3

        rows = list(seq.query(db, limit=1))
        assert len(rows) == 1


def test_pformat_empty_and_nonempty():
    class Foo(minidb.Model):
        value = int

    with minidb.Store(debug=True) as db:
        db.register(Foo)
        assert minidb.pformat(Foo.query(db, Foo.c.value)) == '(no rows)'

        Foo(value=1).save(db)
        text = minidb.pformat(Foo.query(db, Foo.c.id // Foo.c.value), color=True)
        assert 'row(s)' in text


def test_literal_and_function_repr_and_tosql():
    lit = minidb.literal('CURRENT_TIMESTAMP')
    assert repr(lit) == 'CURRENT_TIMESTAMP'

    fn = minidb.func.max(lit)
    assert 'max(' in repr(fn)
...
def test_datetime_converters_with_microseconds():
    dt = datetime.datetime(2024, 1, 2, 3, 4, 5, 123456)
    tm = datetime.time(3, 4, 5, 654321)

    dt_s = minidb.convert_datetime_datetime(dt, True)
    tm_s = minidb.convert_datetime_time(tm, True)

    assert minidb.convert_datetime_datetime(dt_s, False) == dt
    assert minidb.convert_datetime_time(tm_s, False) == tm


def test_model_delete_cache_miss_and_pquery(capsys):
    class Foo(minidb.Model):
        value = int

    with minidb.Store(debug=True) as db:
        db.register(Foo)
        obj = Foo(value=5).save(db)

        # 手动从缓存移除，覆盖 delete 的 cache-miss 分支
        if obj.id in Foo.__minidb_cache__:
            del Foo.__minidb_cache__[obj.id]

        obj.delete()
        assert obj.id is None

        Foo(value=6).save(db)
        Foo.pquery(db, Foo.c.id // Foo.c.value, color=False)
